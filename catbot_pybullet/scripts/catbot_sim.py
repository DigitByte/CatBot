...
class catbot_sim:
    def __init__(self, urdf_dir, startPos, start_euler, ground, logging=rospy, rgba=[0.2, 0.2, 0.2, 1]):
        ...
        for i in range(len(names)):
            if 'hip' in names[i] or 'knee' in names[i]:
                ...
                # pb.enableJointForceTorqueSensor(self.robot, i, True)
    ...
    def getMotorJointStates(self):
        ...
        # print('{:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f}'.format(torques[0], torques[1], torques[2], torques[3], torques[4], torques[5]))
    ...
    def getLidarData(self, max_dist, res_deg):
        ...
        final_pos = np.array( [max_dist * np.cos(all_angles), max_dist * np.sin(all_angles), (lin_pos[2]+0.3)*np.ones( len(all_angles) )  ] )
        lidar_pos = np.array( [lin_pos[0]*np.ones( len(all_angles) ), lin_pos[1]*np.ones( len(all_angles) ), (lin_pos[2]+0.3)*np.ones( len(all_angles) ) ] )
        ...
        collisions = np.array(pb.rayTestBatch(lidar_pos.T, final_pos_trans.T))

        if len(collisions.shape) == 0:
            return None, None

        collision_pos = np.asarray([*collisions[:,3]])


        distance = np.sqrt(  np.power( collision_pos[:,0] - lin_pos[0],2)+ np.power( collision_pos[:,1] - lin_pos[1],2) + np.power( collision_pos[:,2] - lin_pos[2],2)  )


        return all_angles, distance


    def setJointPosition(self, joint_pos, velocity=0, kp=1, kv=0.3):

        vec = [velocity] * self.num_joints
        if len(joint_pos) == 12:
            ...
            for i in range(len(self.index_revolute_joints)):
                ...
                # rospy.loginfo(self.getJointNames()[joint_ix] + ' ' + str(joint_pos[i]))
        else:
            rospy.logerr('Expected position array of length {}, got {}'.format(self.num_joints, len(joint_pos)))
